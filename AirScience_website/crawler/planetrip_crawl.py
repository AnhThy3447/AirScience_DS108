...
def crawl_planetrip(url, brand):
    ...
    df = pd.DataFrame(columns=['flight_id', 'brand', 'price', 'start_time', 'start_day', 'end_time', 'end_day', 'destination',
                                'trip_time', 'hand_luggage', 'checked_baggage', 'crawl_date'])
    ...
    for i, j in zip(range(len(elements)), detail):
        ...
        # id_element = wait.until(EC.visibility_of_element_located((
        #     By.XPATH, 
        #     "//div[@class='css-901oao r-uh8wd5 r-1b43r93 r-majxgm r-rjixqe r-14gqq1x r-fdjqy7']"
        #     )))
        # text = id_element.text
        # text = text.split('•')[0].strip()
        # if text:
        #     flight_id.append(text)

    new_df = pd.DataFrame(list(zip(flight_id, brand, price, start_time, start_day, end_time, 
                                    end_day, trip_time, destination, 
                                    hand_luggage, checked_baggage, crawl_date)),
                            columns=['flight_id', 'brand', 'price','start_time','start_day', 'end_time', 
                                        'end_day', 'trip_time', 'destination', 
                                        'hand_luggage', 'checked_baggage', 'crawl_date'])
...
def filter_user_inputs(filename='user_input.csv'):
    ...
    def check_luggage(row, hand_luggage, checked_baggage):
        return (row['hand_luggage'] >= hand_luggage) and (row['checked_baggage'] >= checked_baggage)
    df = df[df.apply(lambda row: check_luggage(row, hand_luggage, checked_baggage), axis=1)]
